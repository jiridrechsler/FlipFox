...
function buildCardFor(poolItem: PoolItem) {
    ...
    switch (pref) {
        case "emoji-to-word": return { prompt: emoji || "—", answer: en };
        case "word-to-emoji": return { prompt: en,        answer: emoji || "—" };
        case "word-to-num":   return { prompt: en,        answer: String(num) };
        case "num-to-word":
        default:              return { prompt: String(num), answer: en };
    }
}
...
export function GameProvider({ children }: { children: React.ReactNode }) {
    ...
    const stopRevealTimer = () => { if (revealTimerRef.current) { clearInterval(revealTimerRef.current); revealTimerRef.current = null; } };
    const stopHoldTimers  = () => {
        if (holdTimerRef.current) { clearInterval(holdTimerRef.current); holdTimerRef.current = null; }
        if (holdEndRef.current)   { clearTimeout(holdEndRef.current); holdEndRef.current = null; }
    };
